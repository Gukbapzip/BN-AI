...
extern bool curl_transport_localhost_keepalive( const ai_bridge::request&, const ai_bridge::options&, std::string&, ai_bridge::error_info& );
...
auto get_talk_ai_options() -> ai_bridge::options &
{
    ...
    if( !inited ) {
        ...
        // Transport is intentionally left unset here; wire it from an integration unit that
        // provides the actual HTTP client and assigns opts.transport.
        opts.transport = [](const ai_bridge::request& req, std::string& raw_out, ai_bridge::error_info& err) {
            return curl_transport_localhost_keepalive(req, get_talk_ai_options(), raw_out, err);
        };
        inited = true;
    }
    return opts;
...
auto execute_ai_command_allowlist( dialogue &/*d*/, const ai_bridge::command &/*cmd*/ ) -> void
{
    // Stub: implement an explicit allowlist mapping here.
}
                          ...
                          const talk_topic &topic )
{
    ...
    const auto use_ai = ai_enabled && ( topic.id == "TALK_FRIEND" || topic.id == "TALK_STRANGER_NEUTRAL" );
    ...
    if( use_ai && ai_state.inflight_id == 0 && !ai_state.have_reply ) {
        ...
        req.system =
            "You are an NPC in Cataclysm: Bright Nights. "
            "Return either plain text, or JSON {\"text\":\"...\",\"commands\":[{\"action\":\"...\",\"target\":\"...\",\"args\":{...}}]}.";

        req.ctx.game_state_summary = "";
        req.ctx.proximity_npcs_summary = "";
        req.ctx.player_status_summary = "";
        req.ctx.conversation_summary = "";

        req.user = "Generate the NPC's next reply for topic: " + topic.id;

        ai_state.inflight_id = bridge.enqueue( std::move( req ), [dlg = this]( const ai_bridge::completion &c ) {
    ...
    if( !use_ai || ai_state.last_history_line != challenge ) {
        if( challenge != "\"&NPC is thinking…\"" && challenge != "&NPC is thinking…" ) {
        ...
        do {
            ...
            if( use_ai ) {
                if( ch == KEY_ENTER || ch == '\n' || ch == '\r' ) {
                    ...
                    if( !user_text.empty() ) {
                        d_win.add_to_history( string_format( pgettext( "you say something", "%s: %s" ),
                        ...
                        auto req = ai_bridge::request{};
                        req.timeout = std::chrono::seconds( 30 ); // 타임아웃 설정 (무한 멈춤 방지)
                        req.system =
                            "You are an NPC in Cataclysm: Bright Nights. "
                            "Return either plain text, or JSON {\"text\":\"...\",\"commands\":[...]}. ";
                        req.ctx.game_state_summary = "";
                        req.ctx.proximity_npcs_summary = "";
                        req.ctx.player_status_summary = "";
                        req.ctx.conversation_summary = "";
                        ...
                        ai_state.inflight_id = bridge.enqueue( std::move( req ),
                            [dlg = this]( const ai_bridge::completion &c ) {
                                auto &st = get_talk_ai_state( dlg );
                                st.inflight_id = 0;
                                st.have_reply = true;
                                st.reply_displayed = false;
                                ...
                                if ( c.result.ok ) {
                                    ...
                                    st.pending_cmds = c.result.response.commands;
                                    // NPC의 대화 누적
                                    st.accumulated_chat += "NPC: " + c.result.response.text + "\n";
                                } else {
                                    // 파싱/통신 실패 시 에러 원인 출력
                                    st.reply_text = "&[System] AI 에러: " + c.result.error.message;
