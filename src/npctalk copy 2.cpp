...
static const zone_type_id
    zone_type_npc_investigate_only("NPC_INVESTIGATE_ONLY");
...
extern bool curl_transport_localhost_keepalive(const ai_bridge::request &,
                                               const ai_bridge::options &,
                                               std::string &,
                                               ai_bridge::error_info &);

namespace {

struct talk_ai_state {
  ai_bridge::request_id inflight_id = 0;
  bool have_reply = false;
  bool reply_displayed = false;
...
auto get_talk_ai_options() -> ai_bridge::options & {
  ...
  if (!inited) {
    ...
    opts.transport = [](const ai_bridge::request &req, std::string &raw_out,
                        ai_bridge::error_info &err) {
      return curl_transport_localhost_keepalive(req, get_talk_ai_options(),
                                                raw_out, err);
    };
    inited = true;
  }
...
auto get_talk_ai_bridge() -> ai_bridge & {
  static ai_bridge bridge(get_talk_ai_options());
  return bridge;
}

auto get_talk_ai_state_map() -> std::unordered_map<int, talk_ai_state> & {
  static std::unordered_map<int, talk_ai_state> states;
  return states;
}

auto get_talk_ai_state(const dialogue *d) -> talk_ai_state & {
  // beta(NPC)의 정수 ID를 키로 사용
  return get_talk_ai_state_map()[d->beta->getID().get_value()];
}

auto execute_ai_command_allowlist(dialogue &d, const ai_bridge::command &cmd)
    -> void {
  // 명령어 허용 목록 (AI가 요청한 action 문자열 → 실제 엔진 함수 매핑)
  // 목록에 없는 명령어는 무시된다 (보안상 중요).
  using fn_t = std::function<auto(npc &)->void>;
  static const auto allowlist = std::unordered_map<std::string, fn_t>{
      // === 이동 / 추종 ===
      {"follow", talk_function::follow},           // 정식 동료 편입
      {"follow_only", talk_function::follow_only}, // 단순 추종 (동료 미편입)
      {"stop_following",
       talk_function::stop_following}, // 추종 중단 (비동료 한정)

      // === 경비 ===
      {"guard", talk_function::assign_guard},    // 현 위치 경비 배치
      {"stop_guard", talk_function::stop_guard}, // 경비 해제 → 재추종

      // === 아이템 / 지원 ===
      {"give_equipment", talk_function::give_equipment}, // 무작위 아이템 증정
      ...
      {"do_construction", talk_function::do_construction}, // 건설
      {"do_farming", talk_function::do_farming},           // 농업
      {"do_fishing", talk_function::do_fishing},           // 낚시
      {"do_butcher", talk_function::do_butcher},           // 해체
      {"do_mining", talk_function::do_mining},             // 채광
      {"do_chop_trees", talk_function::do_chop_trees},     // 벌목
      {"do_read", talk_function::do_read},                 // 독서
      {"revert_activity", talk_function::revert_activity}, // 현재 작업 취소
  ...
  const auto it = allowlist.find(cmd.action);
  if (it != allowlist.end()) {
    it->second(*npc_ptr);
  }
}
  ...
  auto get_weapon_info = [](const item &weap,
    ...
    if (weap.is_gun()) {
      ...
      return string_format("%s (Melee, Bash: %d, Pierce: %d, Cut: %d)",
                           ...
                           weap.damage_melee(damage_type::DT_CUT));
    }
  };
...
std::string talk_trial::name() const {
  static const std::array<std::string, NUM_TALK_TRIALS> texts = {
      {"", translate_marker("LIE"), translate_marker("PERSUADE"),
       translate_marker("INTIMIDATE"), ""}};
...
int calc_skill_training_cost(const npc &p, const skill_id &skill) {
  ...
  return 1000 * (1 + you.get_skill_level(skill)) *
         (1 + you.get_skill_level(skill));
}
...
// TODO: maybe move this function into the ma_style class? Or into the NPC
// class?
time_duration calc_ma_style_training_time(const npc &,
                                          const matype_id & /* id */) {
  return 30_minutes;
}
...
// everyone == true adds another entry at the end to allow selecting all listed
// NPCs this implies a return value of npc_list.size() means "everyone"
static int npc_select_menu(const std::vector<npc *> &npc_list,
...
static void npc_batch_override_toggle(const std::vector<npc *> &npc_list,
                                      ally_rule rule, bool state) {
  for (npc *p : npc_list) {
...
static void npc_temp_orders_menu(const std::vector<npc *> &npc_list) {
  ...
  while (!done) {
    ...
    std::string output_string =
        string_format(_("%s currently has these temporary orders:"), guy->name);
    for (const auto &rule : ally_rule_strs) {
      if (guy->rules.has_override_enable(rule.second.rule)) {
        override_count++;
        output_string += "\n  ";
        output_string += (guy->rules.has_override(rule.second.rule)
                              ? rule.second.rule_true_text
    ...
    nmenu.addentry(NPC_CHAT_FORBID_ENGAGE, true, 'f',
                   guy->rules.has_override_enable(ally_rule::forbid_engage)
                       ? _("Go back to your usual engagement habits")
                       : _("Don't engage hostiles for the time being"));
    nmenu.addentry(NPC_CHAT_NO_GUNS, true, 'g',
                   guy->rules.has_override_enable(ally_rule::use_guns)
                       ? _("Use whatever weapon you normally would")
                       : _("Don't use ranged weapons for a while"));
    nmenu.addentry(NPC_CHAT_PULP, true, 'p',
                   guy->rules.has_override_enable(ally_rule::allow_pulp)
                       ? _("Pulp zombies if you like")
                       : _("Hold off on pulping zombies for a while"));
    nmenu.addentry(NPC_CHAT_FOLLOW_CLOSE, true, 'c',
                   guy->rules.has_override_enable(ally_rule::follow_close) &&
                       ...
                       ? _("Go back to keeping your usual distance")
                       : _("Stick close to me for now"));
    nmenu.addentry(NPC_CHAT_MOVE_FREELY, true, 'm',
                   guy->rules.has_override_enable(ally_rule::follow_close) &&
                           !guy->rules.has_override(ally_rule::follow_close)
                       ? _("Go back to keeping your usual distance")
                       : _("Move farther from me if you need to"));
    nmenu.addentry(NPC_CHAT_SLEEP, true, 's',
                   guy->rules.has_override_enable(ally_rule::allow_sleep)
                       ? _("Go back to your usual sleeping habits")
                       : _("Take a nap if you need it"));
    nmenu.addentry(NPC_CHAT_CLEAR_OVERRIDES, true, 'o',
                   _("Let's go back to your usual behaviors"));
    nmenu.query();
...
void game::chat() {
  ...
  if (u.has_trait(trait_PROF_FOODP) &&
      !(u.is_wearing(itype_id("foodperson_mask")) ||
  ...
  if (!available.empty()) {
    nmenu.addentry(NPC_CHAT_TALK, true, 't',
                   available_count == 1
                       ? string_format(_("Talk to %s"), available.front()->name)
                       : _("Talk to…"));
  }
  nmenu.addentry(NPC_CHAT_YELL, true, 'a',
                 u.has_trait(trait_SHOUT2)   ? _("Scream")
                 : u.has_trait(trait_SHOUT3) ? _("Howl")
                                             : _("Yell"));
  ...
  if (!animal_vehicles.empty()) {
    nmenu.addentry(
        NPC_CHAT_ANIMAL_VEHICLE_FOLLOW, true, 'F',
  ...
  if (!magic_vehicles.empty()) {
    nmenu.addentry(NPC_CHAT_COMMAND_MAGIC_VEHICLE_FOLLOW, true, 'Q',
                   _("Utter a magical command that will order your magical "
                     "vehicles to follow you."));
  }
  if (!magic_following_vehicles.empty()) {
    nmenu.addentry(NPC_CHAT_COMMAND_MAGIC_VEHICLE_STOP_FOLLOW, true, 'q',
                   _("Utter a magical command that will order your magical "
                     "vehicles to stop following you."));
  }
  if (!following_vehicles.empty()) {
    nmenu.addentry(
        NPC_CHAT_ANIMAL_VEHICLE_STOP_FOLLOW, true, 'S',
        _("Whistle at your animals pulling vehicles to stop following you."));
  }
  if (!guards.empty()) {
    nmenu.addentry(NPC_CHAT_FOLLOW, true, 'f',
                   guard_count == 1 ? string_format(_("Tell %s to follow"),
                                                    guards.front()->name)
                                    : _("Tell someone to follow…"));
  }
  if (!followers.empty()) {
    nmenu.addentry(
        NPC_CHAT_CONTROL, true, 'C',
        follower_count == 1
            ? string_format(_("Control %s"), followers.front()->get_name())
            : _("Control someone…"));
    nmenu.addentry(NPC_CHAT_GUARD, true, 'g',
                   follower_count == 1 ? string_format(_("Tell %s to guard"),
                                                       followers.front()->name)
                                       : _("Tell someone to guard…"));
    nmenu.addentry(NPC_CHAT_MOVE_TO_POS, true, 'G',
                   follower_count == 1
                       ? string_format(_("Tell %s to move to location"),
                                       followers.front()->get_name())
                       : _("Tell someone to move to location…"));
    nmenu.addentry(NPC_CHAT_AWAKE, true, 'w',
                   _("Tell everyone on your team to wake up"));
    nmenu.addentry(NPC_CHAT_MOUNT, true, 'M',
                   _("Tell everyone on your team to mount up"));
    nmenu.addentry(NPC_CHAT_DISMOUNT, true, 'm',
                   _("Tell everyone on your team to dismount"));
    ...
    nmenu.addentry(NPC_CHAT_ORDERS, true, 'o',
                   _("Tell everyone on your team to temporarily…"));
  }
  std::string yell_msg;
  ...
  case NPC_CHAT_EMOTE_OVERLAY: {
    ...
    std::map<int, efftype_id>
        dynamic_emote_key_map; // Map from menu key (int) to efftype_id
    ...
    for (const efftype_id &effect_id : all_effects) {
      ...
      effect temp_effect(&etype, 0_turns, bodypart_str_id::NULL_ID(), 1,
                         calendar::turn_zero);
    ...
    if (dynamic_emote_key_map.contains(emenu.ret)) {
      selected_effect_id =
          dynamic_emote_key_map[emenu.ret]; // Get the efftype_id using the int
                                            // key
    }
  ...
  case NPC_CHAT_MONOLOGUE: {
    ...
    // Build help text
    const auto &help_fmt = _("<color_light_gray>You can add a prefix to your "
                             "monologue to set the tone or emotion."
                             " Valid prefixes are:</color> %s\n"
                             "\n"
                             "<color_white>Examples:</color>\n"
                             "  <color_light_green>good: You feel like it's "
                             "going to be a good day.</color>\n"
                             "  <color_light_red>bad: You don't like the look "
                             "of this place…</color>\n"
                             "  <color_light_blue>info: The empty house "
                             "reminds you of old times.</color>\n"
                             "  <color_yellow>warning: You say 'Stay close, I "
                             "hear something moving!'</color>\n"
                             "\n"
                             "<color_light_gray>Leave off the prefix for a "
                             "neutral monologue.</color>\n");
    ...
    for (auto it = type_list.begin(); it != type_list.end(); ++it) {
      if (debug_mode || it->first != m_debug) {
        const auto &col_name =
            get_all_colors().get_name(msgtype_to_color(it->first));
        type_strings.push_back(
            string_format(pgettext("message log", "<color_%s>%s</color>"),
  ...
  case NPC_CHAT_GUARD: {
    const int npcselect =
        npc_select_menu(followers, _("Who should guard here?"));
    if (npcselect < 0) {
      return;
    }
    if (npcselect == follower_count) {
      ...
      yell_msg =
          string_format(_("Guard here, %s!"), followers[npcselect]->name);
    }
    break;
  }
  case NPC_CHAT_MOVE_TO_POS: {
    ...
    if (npcselect == follower_count) {
      ...
      yell_msg =
          string_format(_("Move there, %s!"), followers[npcselect]->get_name());
    }
    break;
  }
  case NPC_CHAT_CONTROL: {
    const int npcselect =
        npc_select_menu(followers, _("Who do you want to control?"), false);
    if (npcselect < 0) {
      return;
    }
  ...
  case NPC_CHAT_DANGER: {
    ...
    yell_msg =
        _("We're in danger.  Stay awake, stay close, don't go wandering off, "
...
void npc::handle_sound(const sounds::sound_t spriority,
                       const std::string &description, int heard_volume,
                       const tripoint &spos) {
  map &here = get_map();
  ...
  bool npc_ally =
      sound_source && sound_source->is_npc() && is_ally(*sound_source);
  ...
  if (spriority < sounds::sound_t::alarm && get_avatar().sees(spos)) {
    if (is_player_ally()) {
      add_msg(m_debug, "NPC %s ignored low priority noise that player can see",
              name);
      return;
      // discount if sound source is player, or seen by player,
  ...
  if (mission == NPC_MISSION_GUARD_ALLY ||
  ...
  if (ai_cache.total_danger < 1.0f) {
    if (spriority == sounds::sound_t::movement && !in_vehicle) {
      ...
      if ((spriority == sounds::sound_t::speech ||
           spriority == sounds::sound_t::alert ||
           spriority == sounds::sound_t::order) &&
          sound_source &&
          !has_faction_relationship(*sound_source,
                                    npc_factions::knows_your_voice)) {
      ...
      if (should_check) {
        ...
        if (mgr.has(zone_type_npc_no_investigate, s_abs_pos, fac_id)) {
          should_check = false;
        } else if (mgr.has(zone_type_npc_investigate_only, my_abs_pos,
                           fac_id) &&
                   !mgr.has(zone_type_npc_investigate_only, s_abs_pos,
                            fac_id)) {
          should_check = false;
        }
      }
      if (should_check) {
        add_msg(m_debug, "%s added noise at pos %d:%d", name, s_abs_pos.x,
                s_abs_pos.y);
...
void npc_chatbin::check_missions() {
  ...
  const auto last =
      std::remove_if(ma.begin(), ma.end(),
                     [](class mission const *m) { return !m->is_assigned(); });
...
void npc::talk_to_u(bool radio_contact, bool enforce_first_topic) {
  ...
  // For each active mission we have, let the mission know we talked to this
  // NPC.
  ...
  if (you.has_trait(trait_PROF_FOODP) &&
      !(you.is_wearing(itype_id("foodperson_mask")) ||
  ...
  const auto hook_results =
      cata::run_hooks("on_dialogue_start", [&, this](auto &params) {
        params["npc"] = this;
        params["next_topic"] = d.topic_stack.back().id;
      });
  for (const auto &result : hook_results) {
    if (!result.second.is<sol::table>()) {
      continue;
    };
  ...
  do {
    ...
    const auto hook_results =
        cata::run_hooks("on_dialogue_option", [&, this](auto &params) {
          params["npc"] = this;
    ...
    for (const auto &result : hook_results) {
      ...
      final_result = result.second.as<sol::table>().get_or<std::string>(
          "result", final_result);
      // Allow higher priority topics to veto, but still trigger subsequent
      // calls? auto allowed = result.second.as<sol::table>().get<sol::object>(
      // "allowed" ); if ( allowed.is<bool>() && !allowed.as<bool>() ) { break;
      // };
    }
    ...
    if (next.id == "TALK_DONE" || d.topic_stack.empty()) {
      ...
      if (next.id != d.topic_stack.back().id) {
        d.add_topic(next);
      }
    }
  } while (!d.done);

  cata::run_hooks("on_dialogue_end",
                  [&, this](auto &params) { params["npc"] = this; });
...
std::string dialogue::dynamic_line(const talk_topic &the_topic) const {
  ...
  if (topic == "TALK_NOFACE") {
    ...
    return string_format(_("&You are deaf and can't talk.  When you don't "
                           "respond, %s becomes angry!"),
                         beta->name);
  }
  if (topic == "TALK_SEDATED") {
    int firstaid_lvl =
        get_player_character().get_skill_level(skill_id("firstaid"));
    time_duration dur = character_funcs::estimate_effect_dur(
        firstaid_lvl, effect_narcosis, 15_minutes, 6, *beta);
    return string_format(
        _("%1$s is sedated and can't be moved or woken up until the medication "
          "or sedation wears off.\nYou estimate it will wear off in %2$s."),
  ...
  static const std::unordered_set<std::string> mission_topics = {
      {"TALK_MISSION_DESCRIBE", "TALK_MISSION_DESCRIBE_URGENT",
       ...
       "TALK_MISSION_ADVICE", "TALK_MISSION_INQUIRE", "TALK_MISSION_SUCCESS",
       "TALK_MISSION_SUCCESS_LIE", "TALK_MISSION_FAILURE"}};
  if (mission_topics.contains(topic)) {
    ...
    // TODO: make it a member of the mission class, maybe at mission instance
    // specific data
    const std::string &ret = miss->dialogue_for_topic(topic);
    if (ret.empty()) {
      debugmsg(
          "Bug in npctalk.cpp:dynamic_line.  Wrong mission_id(%s) or topic(%s)",
    ...
    if (topic == "TALK_MISSION_SUCCESS" && miss->has_follow_up()) {
      ...
      case 2:
        return ret + _("  I could use a hand with something else if you are "
                       "interested.");
      case 3:
  ...
  if (topic == "TALK_NONE" || topic == "TALK_DONE") {
    ...
    case NPC_MISSION_GUARD_PATROL:
      return string_format(
          _("Currently, I'm guarding this location.  Overall, %s"),
          p->myclass.obj().get_job_description());
    case NPC_MISSION_ACTIVITY:
      return string_format(
          _("Right now, I'm <current_activity>.  In general, %s"),
          p->myclass.obj().get_job_description());
    ...
    if (ability >= 100) {
      ...
      if (p->max_stored_kcal() - p->get_stored_kcal() > 500) {
        time_duration hunger_at =
            5_minutes * (500 - p->max_stored_kcal() + p->get_stored_kcal()) /
            p->bmr();
  ...
  return string_format(
      "I don't know what to say for %s. (BUG (npctalk.cpp:dynamic_line))",
      topic);
}
...
void dialogue::apply_speaker_effects(const talk_topic &the_topic) {
  ...
  for (json_dynamic_line_effect &npc_effect :
       iter->second.get_speaker_effects()) {
...
talk_response &dialogue::add_response(const std::string &text,
                                      const std::string &r, const bool first) {
  talk_response result = talk_response();
  result.truetext = no_translation(text);
  result.truefalse_condition = [](const dialogue &) { return true; };
...
talk_response &dialogue::add_response(const std::string &text,
                                      const std::string &r, mission *miss,
                                      const bool first) {
  if (miss == nullptr) {
...
talk_response &dialogue::add_response(const std::string &text,
                                      const std::string &r, const spell_id &sp,
                                      const bool first) {
  talk_response &result = add_response(text, r, first);
...
void dialogue::gen_responses(const talk_topic &the_topic) {
  const auto &topic =
      the_topic.id;    // for compatibility, later replace it in the code below
  const auto p = beta; // for compatibility, later replace it in the code below
  auto &ret =
      responses; // for compatibility, later replace it in the code below
  ret.clear();
  const auto iter = json_talk_topics.find(topic);
  if (iter != json_talk_topics.end()) {
  ...
  if (topic == "TALK_MISSION_LIST") {
    if (p->chatbin.missions.size() == 1) {
      ...
      for (auto &mission : p->chatbin.missions) {
        add_response(mission->get_type().tname(), "TALK_MISSION_OFFER", mission,
                     true);
      }
    }
  } else if (topic == "TALK_MISSION_LIST_ASSIGNED") {
    if (missions_assigned.size() == 1) {
      add_response(_("I have news."), "TALK_MISSION_INQUIRE",
                   missions_assigned.front());
    } else {
      for (auto &miss_it : missions_assigned) {
        add_response(miss_it->get_type().tname(), "TALK_MISSION_INQUIRE",
                     miss_it);
      }
    }
  } else if (topic == "TALK_TRAIN") {
      ...
      if (!skillt.is_valid()) {
        ...
        if (!styleid.is_valid()) {
          ...
          if (p->magic->knows_spell(sp_id)) {
            add_response(
                string_format(_("Yes, let's resume training %s"), sp_id->name),
                "TALK_TRAIN_START", sp_id);
          }
        } else {
          const martialart &style = styleid.obj();
          add_response(
              string_format(_("Yes, let's resume training %s"), style.name),
              "TALK_TRAIN_START", style);
        }
      } else {
        add_response(
            string_format(_("Yes, let's resume training %s"), skillt->name()),
    ...
    for (const spell_id &sp : spells) {
      ...
      if (you.magic->can_learn_spell(you, sp)) {
        if (you.magic->knows_spell(sp)) {
          ...
          if (player_spell.is_max_level() ||
              player_spell.get_level() >= temp_spell.get_level()) {
            continue;
          }
        }
        teachable_spells.emplace_back(sp);
    ...
    for (const spell_id &sp : teachable_spells) {
      ...
      const int cost = p->calc_spell_training_cost(
          knows, temp_spell.get_difficulty(), temp_spell.get_level());
      std::string text;
      if (knows) {
        text = string_format(_("%s: 1 hour lesson (cost %s)"),
    ...
    for (auto &style_id : styles) {
      ...
      const std::string text = string_format(
          cost > 0 ? _("%s ( cost $%d )") : "%s", style.name, cost / 100);
    ...
    for (auto &trained : trainable) {
      ...
      //~Skill name: current level (exercise) -> next level (exercise) (cost in
      // dollars)
      std::string text =
          string_format(cost > 0 ? _("%s: %d (%d%%) -> %d (%d%%) (cost $%d)")
                                 : _("%s: %d (%d%%) -> %d (%d%%)"),
...
static int parse_mod(const dialogue &d, const std::string &attribute,
                     const int factor) {
  player &u = *d.alpha;
  npc &p = *d.beta;
  int modifier = 0;
...
int talk_trial::calc_chance(const dialogue &d) const {
  ...
  case TALK_TRIAL_LIE:
    chance += character_effects::talk_skill(u) -
              character_effects::talk_skill(p) + p.op_of_u.trust * 3;
    chance += u_mods.lie;
  ...
  case TALK_TRIAL_PERSUADE:
    chance += character_effects::talk_skill(u) -
              character_effects::talk_skill(p) / 2 + p.op_of_u.trust * 2 +
              p.op_of_u.value;
  ...
  case TALK_TRIAL_INTIMIDATE:
    chance += character_effects::intimidation(u) -
              character_effects::intimidation(p) + p.op_of_u.fear * 2 -
...
int topic_category(const talk_topic &the_topic) {
  ...
  // How this works: each category has a set of topics that belong to it, each
  // set is checked for the given topic and if a set contains, the category
  // number is returned.
  static const std::unordered_set<std::string> topic_1 = {
      {"TALK_MISSION_START", "TALK_MISSION_DESCRIBE", "TALK_MISSION_OFFER",
       ...
       "TALK_MISSION_INQUIRE", "TALK_MISSION_SUCCESS",
       "TALK_MISSION_SUCCESS_LIE", "TALK_MISSION_FAILURE",
       "TALK_MISSION_REWARD", "TALK_MISSION_END",
...
void parse_tags(std::string &phrase, const Character &u, const Character &me,
  ...
  do {
    ...
    if (tag.size() > 12 && tag.substr(0, 11) == "<utalk_var_") {
      ...
      if (u_var.empty()) {
        debugmsg("Player talk variable not found.  '%s'  (%d - %d)",
                 tag.c_str(), fa, fb);
      ...
      if (npc_var.empty()) {
        debugmsg("NPC talk variable not found.  '%s'  (%d - %d)", tag.c_str(),
                 fa, fb);
        phrase.replace(fa, fb - fa + 1, "????");
      } else {
    ...
    // Special, dynamic tags go here
      ...
      std::string npcstr =
          me.male ? pgettext("npc", "He") : pgettext("npc", "She");
      ...
      npc *guy =
          const_cast<npc *>(me.as_npc()); // remove const-ness from pointer
      time_duration const restock_remaining = guy->restock - calendar::turn;
      // reset if the restock rate is higher than the possible max, or null
      // (setting has changed or not oponed shop yet)
      if (restock_remaining < -1_seconds ||
          restock_remaining >
              3_days * get_option<float>("RESTOCK_DELAY_MULT")) {
        guy->restock =
            calendar::turn + 3_days * get_option<float>("RESTOCK_DELAY_MULT");
      }
...
talk_data talk_response::create_option_line(const dialogue &d,
                                            const char letter) {
  std::string ftext;
  ...
  if (consequences.contains(dialogue_consequence::hostile)) {
    color = c_red;
             ...
             consequences.contains(dialogue_consequence::helpless)) {
    color = c_light_red;
  } else if (text[0] == '&' ||
             consequences.contains(dialogue_consequence::action)) {
    color = c_green;
  } else {
    color = c_white;
  }
...
std::set<dialogue_consequence>
talk_response::get_consequences(const dialogue &d) const {
...
talk_topic dialogue::opt(dialogue_window &d_win, const std::string &npc_name,
  ...
  // --- AI dialogue hook (async, zero-freeze) ---
  const auto ai_enabled = static_cast<bool>(get_talk_ai_options().transport) &&
                          !get_talk_ai_options().endpoint.empty();
  static const auto ai_topics = std::unordered_set<std::string>{
      "TALK_FRIEND",           "TALK_FRIEND_GUARD",      "TALK_FRIEND_TIRED",
      "TALK_STRANGER_NEUTRAL", "TALK_STRANGER_FRIENDLY",
  };
  const auto use_ai = ai_enabled && ai_topics.count(topic.id) > 0;
  auto &bridge = get_talk_ai_bridge();
  auto &ai_state = get_talk_ai_state(this);
  ...
  if (challenge[0] == '&') {
    ...
    challenge =
        string_format(pgettext("npc does something", "%s %s"),
                      colorize(beta->name, c_light_green), challenge.substr(1));
  } else {
    challenge = string_format(pgettext("npc says something", "%s: %s"),
                              colorize(beta->name, c_light_green), challenge);
  }

  if (!use_ai || ai_state.last_history_line != challenge) {
    if (challenge != "\"&NPC is thinking…\"" &&
        challenge != "&NPC is thinking…") {
      d_win.add_to_history(challenge);
    }
    if (use_ai) {
      ai_state.last_history_line = challenge;
    }
  }
  ...
  ui_adaptor ui;
  ui.on_screen_resize([&](ui_adaptor &ui) { d_win.resize_dialogue(ui); });
  ui.mark_resize();
  ...
  do {
    ...
    okay = true;
    std::set<dialogue_consequence> consequences =
        responses[ch].get_consequences(*this);
    if (consequences.contains(dialogue_consequence::hostile)) {
  ...
  std::string response_printed =
      string_format(pgettext("you say something", "%s: %s"),
                    colorize(_("You"), c_green), response_lines[ch].text);
...
#define WRAP(value) {#value, TALK_TRIAL_##value}
      WRAP(NONE), WRAP(LIE), WRAP(PERSUADE), WRAP(INTIMIDATE), WRAP(CONDITION)
#undef WRAP
  }};
...
talk_effect_fun_t::talk_effect_fun_t(
    const std::function<void(const dialogue &d)> &fun) {
  function = [fun](const dialogue &d) { fun(d); };
}

// TODO: Remove after effect permanence change
#include "effect.h"
void talk_effect_fun_t::set_add_effect(const JsonObject &jo,
  ...
  if (jo.has_string("duration")) {
    ...
    if (dur_string == "PERMANENT") {
      ...
      if (json_report_strict) {
        ...
        try {
          jo.throw_error("Effect permanence has been moved to effect_type.  "
                         "Set permanence there.",
                         "duration");
...
void talk_effect_fun_t::set_remove_effect(const JsonObject &jo,
...
void talk_effect_fun_t::set_add_trait(const JsonObject &jo,
                                      const std::string &member, bool is_npc) {
...
void talk_effect_fun_t::set_remove_trait(const JsonObject &jo,
                                         const std::string &member,
                                         bool is_npc) {
...
void talk_effect_fun_t::set_assign_mission(const JsonObject &jo,
                                           const std::string &member) {
...
void talk_effect_fun_t::set_finish_mission(const JsonObject &jo,
                                           const std::string &member) {
  std::string mission_name = jo.get_string(member);
...
void talk_effect_fun_t::set_add_var(const JsonObject &jo,
                                    const std::string &member, bool is_npc) {
...
void talk_effect_fun_t::set_remove_var(const JsonObject &jo,
                                       const std::string &member, bool is_npc) {
...
void talk_effect_fun_t::set_adjust_var(const JsonObject &jo,
                                       const std::string &member, bool is_npc) {
  const std::string var_name = get_talk_varname(jo, member, false);
...
void talk_effect_fun_t::set_u_buy_item(const itype_id &item_name, int cost,
                                       int count,
  ...
  function = [item_name, cost, count, container_name](const dialogue &d) {
    ...
    if (container_name.empty()) {
      ...
      detached_ptr<item> container =
          item::spawn(container_name, calendar::turn);
...
void talk_effect_fun_t::set_u_sell_item(const itype_id &item_name, int cost,
                                        int count) {
  function = [item_name, cost, count](const dialogue &d) {
    ...
    if (count == 1) {
      ...
      //~ %1%s is the NPC name, %2$d is a number of items, %3$s are items
      popup(_("You give %1$s %2$d %3$s."), p.name, count,
...
void talk_effect_fun_t::set_consume_item(const JsonObject &jo,
                                         const std::string &member, int count,
                                         bool is_npc) {
  itype_id item_name;
  ...
  function = [is_npc, item_name, count](const dialogue &d) {
    ...
    const auto consume_item = [&](player &p, const itype_id &item_name,
                                  int count) {
...
void talk_effect_fun_t::set_remove_item_with(const JsonObject &jo,
                                             const std::string &member,
                                             bool is_npc) {
...
void talk_effect_fun_t::set_u_spend_ecash(int amount) {
  function = [amount](const dialogue &d) { d.alpha->cash -= amount; };
}

void talk_effect_fun_t::set_npc_change_faction(
...
void talk_effect_fun_t::set_add_debt(
    const std::vector<trial_mod> &debt_modifiers) {
...
void talk_effect_fun_t::set_mapgen_update(const JsonObject &jo,
                                          const std::string &member) {
  mission_target_params target_params =
      mission_util::parse_mission_om_target(jo);
  ...
  function = [target_params, update_ids](const dialogue &d) {
    ...
    const tripoint_abs_omt omt_pos =
        mission_util::get_om_terrain_pos(update_params);
    for (const std::string &mapgen_update_id : update_ids) {
      run_mapgen_update_func(mapgen_update_id, omt_pos,
                             d.beta->chatbin.mission_selected);
    }
  };
}
...
void talk_effect_fun_t::set_bulk_trade_accept(bool is_trade, bool is_npc) {
  function = [is_trade, is_npc](const dialogue &d) {
    ...
    if (is_trade) {
      ...
      if (d.beta->get_faction() &&
          !d.beta->get_faction()->currency.is_empty()) {
        ...
        if (d.beta->value(*pay) > 0) {
          ...
          if (is_npc) {
            ...
            if (buyer_has == 1) {
              ...
              //~ %1%s is the NPC name, %2$d is a number of items, %3$s are
              // items
              popup(_("%1$s gives you %2$d %3$s."), d.beta->disp_name(),
                    buyer_has, pay->tname());
            }
          }
...
void talk_effect_fun_t::set_add_mission(const std::string &mission_id) {
  function = [mission_id](const dialogue &d) {
    npc &p = *d.beta;
    mission *miss =
        mission::reserve_new(mission_type_id(mission_id), p.getID());
    miss->assign(get_avatar());
    p.chatbin.missions_assigned.push_back(miss);
  };
}

const std::vector<std::pair<int, itype_id>> &
talk_effect_fun_t::get_likely_rewards() const {
  return likely_rewards;
}

void talk_effect_fun_t::set_u_buy_monster(const std::string &monster_type_id,
                                          int cost, int count, bool pacified,
  ...
  function = [monster_type_id, cost, count, pacified, name](const dialogue &d) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!mon_ptr) {
        add_msg(m_debug,
                "Cannot place u_buy_monster, no valid placement locations.");
        break;
      }
      monster &tmp = *mon_ptr;
    ...
    if (name.empty()) {
      popup(_("%1$s gives you %2$d %3$s."), p.name, count,
            mtype.obj().nname(count));
    } else {
...
void talk_effect_fun_t::set_u_learn_recipe(
    const std::string &learned_recipe_id) {
...
void talk_effect_t::set_effect_consequence(
    const std::function<void(npc &p)> &ptr, dialogue_consequence con) {
...
void talk_effect_t::set_effect(const talk_effect_fun_t &fun) {
  effects.push_back(fun);
  guaranteed_consequence =
      std::max(guaranteed_consequence, dialogue_consequence::none);
}
...
talk_topic talk_effect_t::apply(dialogue &d) const {
  // Need to get a reference to the mission before effects are applied, because
  // effects can remove the mission
  ...
  if (miss && (mission_opinion.trust || mission_opinion.fear ||
    ...
    npc_opinion mod = npc_opinion(
        mission_opinion.trust ? m_value / mission_opinion.trust : 0,
        mission_opinion.fear ? m_value / mission_opinion.fear : 0,
        mission_opinion.value ? m_value / mission_opinion.value : 0,
        mission_opinion.anger ? m_value / mission_opinion.anger : 0, 0);
    d.beta->op_of_u += mod;
  }
  ...
  // TODO: this is a hack, it should be in clear_mission or so, but those
  // functions have no access to the dialogue object.
  auto &ma = d.missions_assigned;
  ma.clear();
  // Update the missions we can talk about (must only be current, non-complete
  // ones)
...
void talk_effect_t::parse_sub_effect(const JsonObject &jo) {
  ...
  if (jo.has_string("u_add_effect")) {
    ...
    subeffect_fun.set_u_spend_ecash(cash_change);
             ...
             jo.has_string("u_consume_item") ||
             jo.has_string("npc_consume_item") ||
             jo.has_string("u_remove_item_with") ||
             jo.has_string("npc_remove_item_with")) {
    int cost = 0;
    if (jo.has_int("cost")) {
    ...
    translation name;
    jo.read("name", name);
    subeffect_fun.set_u_buy_monster(monster_type_id, cost, count, pacified,
                                    name);
...
void talk_effect_t::parse_string_effect(const std::string &effect_id,
                                        const JsonObject &jo) {
  static const std::unordered_map<std::string, void (*)(npc &)>
      static_functions_map = {{
...
#undef WRAP
  ...
  talk_effect_fun_t subeffect_fun;
  if (effect_id == "u_bulk_trade_accept" ||
      effect_id == "npc_bulk_trade_accept" || effect_id == "u_bulk_donate" ||
      effect_id == "npc_bulk_donate") {
    bool is_npc =
        effect_id == "npc_bulk_trade_accept" || effect_id == "npc_bulk_donate";
    bool is_trade = effect_id == "u_bulk_trade_accept" ||
                    effect_id == "npc_bulk_trade_accept";
...
talk_response::talk_response() {
  truefalse_condition = [](const dialogue &) { return true; };
...
talk_response::talk_response(const JsonObject &jo) {
  if (jo.has_member("truefalsetext")) {
    ...
    read_condition<dialogue>(truefalse_jo, "condition", truefalse_condition,
                             true);
  ...
  if (trial && !jo.has_member("failure")) {
    jo.throw_error(
        "the failure effect is mandatory if a talk_trial has been defined");
  }
...
bool json_talk_response::gen_repeat_response(dialogue &d,
...
static std::string
translate_gendered_line(const std::string &line,
  ...
  for (const std::string &subject : relevant_genders) {
    if (subject == "npc") {
      ...
      debugmsg("Unsupported subject '%s' for grammatical gender in dialogue",
               subject);
    }
  }
...
dynamic_line_t::dynamic_line_t(const std::string &line) {
  function = [line](const dialogue &) { return _(line); };
}
...
dynamic_line_t::dynamic_line_t(const JsonObject &jo) {
  if (jo.has_member("and")) {
    ...
    function = [&](const dialogue &) { return get_hint(); };
    ...
    for (const std::string gender : jo.get_array("relevant_genders")) {
      ...
      if (gender != "npc" && gender != "u") {
        jo.throw_error(
            "Unexpected subject in relevant_genders; expected 'npc' or 'u'");
      }
    }
...
json_dynamic_line_effect::json_dynamic_line_effect(const JsonObject &jo,
  ...
  // if the topic has a sentinel, it means implicitly add a check for the
  // sentinel value and do not run the effects if it is set.  if it is not set,
  // run the effects and set the sentinel
  if (jo.has_string("sentinel")) {
    ...
    std::function<void(const dialogue &d)> function =
        [varname](const dialogue &d) { d.alpha->set_value(varname, "yes"); };
...
void json_dynamic_line_effect::apply(dialogue &d) const { effect.apply(d); }
...
void json_talk_topic::load(const JsonObject &jo) {
  ...
  replace_built_in_responses =
      jo.get_bool("replace_built_in_responses", replace_built_in_responses);
}

bool json_talk_topic::gen_responses(dialogue &d) const {
  d.responses.reserve(
      responses.size()); // A wild guess, can actually be more or less
  ...
  for (const json_talk_repeat_response &repeat : repeat_responses) {
    ...
    for (const itype_id &item_id : repeat.for_item) {
      if (actor->charges_of(item_id) > 0 || actor->has_amount(item_id, 1)) {
        switch_done |=
            repeat.response.gen_repeat_response(d, item_id, switch_done);
      }
    }
    for (const item_category_id &category_id : repeat.for_category) {
      const bool include_containers = repeat.include_containers;
      const auto items_with =
          actor->items_with([category_id, include_containers](const item &it) {
      ...
      for (const auto &it : items_with) {
        switch_done |=
            repeat.response.gen_repeat_response(d, it->typeId(), switch_done);
      }
    }
  }
...
std::vector<json_dynamic_line_effect>
json_talk_topic::get_speaker_effects() const {
  return speaker_effects;
}

void json_talk_topic::check_consistency() const {
  // TODO: check that all referenced topic actually exist. This is currently not
  // possible as they only exist as built in strings, not in the
  // json_talk_topics map.
}

void unload_talk_topics() { json_talk_topics.clear(); }
...
std::string npc::pick_talk_topic(const Character &) {
  ...
  if (op_of_u.trust - op_of_u.fear +
          (personality.bravery + personality.altruism) / 2 >
      0) {
...
static consumption_result try_consume(npc &p, item &it, std::string &reason) {
  ...
  // If not consuming contents and charge <= 0, we just ate the last charge from
  // the stack
  return CONSUMED_ALL;
}
...
std::string give_item_to(npc &p, bool allow_use) {
  ...
  const double cur_weapon_value =
      npc_ai::weapon_value(p, p.primary_weapon(), our_ammo);
  ...
  if (allow_use) {
    ...
    } // HACK: is_gun here is a hack to prevent NPCs wearing guns if they don't
      // want to use them
    ...
    if (p.can_pick_volume(given) && p.can_pick_weight(given)) {
      ...
      if (!p.can_pick_volume(given)) {
        ...
        if (free_space > 0_ml) {
          reason +=
              string_format(_("I can only store %s %s more."),
